...
namespace IrrAdapter{
	Movie::Movie(std::shared_ptr<Image> & output, int w, int h) : m_Image(nullptr), m_Raw(nullptr), m_W(w), m_H(h){
	...
	bool Movie::refresh(){
		...
		if(m_Raw!=nextRaw){
			auto image = GetSingleton<IrrApp>()->accessVideoDriver()->createImageFromData(
			...
			m_Image = std::shared_ptr<irr::video::IImage>(image, IrrSafeRelease());
			m_Raw = nextRaw;
			*m_Output = Image(GetSingleton<IrrApp>()->accessVideoDriver()->addTexture("movie", m_Image.get()));
		}
